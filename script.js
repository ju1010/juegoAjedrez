  ...
  methods: {
      ...
      switch (piecetype) {
        ...
        case 'w_pawn':
            ...
            coordinates = coordinates.filter(val => {
              ...
              if (coordinate[0] < sp.x || coordinate[0] > sp.x){ // si la coordenada está a ambos lados del centro, verifique si tiene una pieza oponente;
                ...
                if (coordinate[1] == (parseInt(sp.y) + 2) && $('#' + sp.x + '_' + (parseInt(sp.y) + 1)).attr('chess') != 'null'){
                  // no hacer nada si este es el primer movimiento de los peones y hay una pieza delante de la segunda coordenada;
                } else {
                  return ($('#' + val).attr('chess') == 'null'); // de lo contrario, devuelve la coordenada si no hay ninguna pieza de ajedrez en ella;
                }
              }
        ...
        case 'b_pawn':
          ...
          coordinates = coordinates.filter(val => {
            ...
            if (coordinate[0] < sp.x || coordinate[0] > sp.x){ //si la coordenada está a ambos lados del centro, verifique si tiene una pieza oponente;
              return ($('#' + val).attr('chess') != 'null' && ($('#' + val).attr('chess')).slice(0,1) == 'w'); // devolver coordenadas con piezas oponentes sobre ellas
            } else { // else if si la coordenada está en el centro;
              if (coordinate[1] == (parseInt(sp.y) - 2) && $('#' + sp.x + '_' + (parseInt(sp.y) - 1)).attr('chess') != 'null'){
                // no hacer nada si este es el primer movimiento de los peones y hay una pieza delante de la segunda coordenada;
              } else {
                return ($('#' + val).attr('chess') == 'null'); // de lo contrario, devuelve la coordenada si no hay ninguna pieza de ajedrez en ella;
    ...
    w_options: function (position,coordinates) {
      ...
      coordinates = coordinates.map(function(val){ // convertir x,y en coordenadas de identificación de cuadrícula reales;
        ...
        }).filter(val => { // algoritmo para determinar las opciones de movimiento en la línea de visión para alfil/torre/reina;
          if (flag == false){
...
$(document).ready(function() {
  ...
  $('.gamecell').click(function(e) {
    ...
    if (main.variables.selectedpiece == '' && target.name.slice(0,1) == main.variables.turn) { // show options
    ...
    } else if (main.variables.selectedpiece !='' && target.name == 'null') { // mover pieza seleccionada
      ...
      } else { // de lo contrario, si selecedpiece.name no es el rey blanco/negro, entonces se mueve
    ...
    } else if (main.variables.selectedpiece !='' && target.name != 'null' && target.id != selectedpiece.id && selectedpiece.name.slice(0,1) != target.name.slice(0,1)){ // capturar una pieza
      
      if (selectedpiece.id != target.id && main.variables.highlighted.indexOf(target.id) != (-1)) { // si no está intentando capturar piezas que no estén en su rango de movimiento
    ...
    } else if (main.variables.selectedpiece !='' && target.name != 'null' && target.id != selectedpiece.id && selectedpiece.name.slice(0,1) == target.name.slice(0,1)){ // toggle opciones de mover
